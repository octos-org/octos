...
- Protocol: `loop/create`, `loop/list`, `loop/pause`, `loop/resume`, `loop/delete`, `loop/fire_now` (**request** an immediate fire — it runs through the loop's fire policy and can be rejected if the loop is paused/exhausted or deduplicated, so inspect the returned `fire.queued`/error result rather than assuming a run happened). Notifications `loop/fired`, `loop/updated`. Feature flags: **both** `coding.autonomy.v1` **and** `coding.loop_runtime.v1`.
