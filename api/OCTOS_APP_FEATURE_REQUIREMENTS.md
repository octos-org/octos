...
| APP-003 | Capability negotiation | P0 | App requests and parses supported AppUI features including typed approvals, pane snapshots (`pane.snapshots.v1`), workspace cwd (`session.workspace_cwd.v1`), task output (honouring the result's `is_snapshot_projection` flag), and task-control (`harness.task_control.v1`) when available. Unknown features are ignored safely. | transport unit tests |
...
| APP-017 | Task output read | P0 | App can request `task/output/read`, preserve output cursor per task, cap buffers, and avoid duplicate output after reconnect. App must branch on the result's `is_snapshot_projection` flag: it appends `task/output/delta` only when live-tail is actually supported, and otherwise treats the read as a bounded snapshot (no delta-append assumption). | store + transport tests |
| APP-018 | Task control readiness | P1 | When the server advertises `harness.task_control.v1`, app exposes list/cancel/restart-from-node affordances through AppUI. When absent, controls are hidden or disabled with explanation. | capability tests |
