...
**每日笔记**（`.octos/memory/YYYY-MM-DD.md`）提供近期活动的滚动窗口。最近 **7 天**的每日笔记会自动纳入智能体上下文。这些文件可以手动创建、通过 `write_file` 工具生成，或由下文的**记忆刷新流水线**自动填充。

两个文件始终可以手动编辑——刷新流水线将 MEMORY.md 视为权威来源，在整合时不会覆盖你的手动修改。

### 自动记忆刷新（抽取 + 整合）

Octos 内置一套自动记忆流水线，从你的对话中读取持久化事实并整合进 `MEMORY.md` 与每日笔记——无需手动编辑文件即可让长期记忆自动生长。该功能**默认开启**。
