    ...
    fn description(&self) -> &str {
        if self.ir_enabled {
            "Run a multi-step pipeline, either by NAME or by composing one. \
             ...
             directly. Use `ir` to offload research→synthesize, code \
             review→edit→verify, or parallel fan-out→converge work to the \
             harness. If composition is invalid the \
    ...
    fn input_schema(&self) -> serde_json::Value {
        ...
        if self.ir_enabled {
            schema["properties"]["ir"] = serde_json::json!({
                "type": "string",
                "description": "A typed-IR workflow as a JSON string. Shape: {\"id\":\"<name>\",\"nodes\":[{\"id\":\"<nid>\",\"kind\":<KIND>}],\"edges\":[{\"source\":\"a\",\"target\":\"b\",\"condition\":\"<opt>\"}]}. <KIND> is EXACTLY one of (tagged by \"type\", no other fields): {\"type\":\"research\",\"prompt\":\"...\"} (web+file read, cheap), {\"type\":\"transform\",\"prompt\":\"...\"} (pure data transform, cheap), {\"type\":\"synthesize\",\"prompt\":\"...\"} (read-only writeup, strong), {\"type\":\"report\",\"prompt\":\"...\"} (final writeup that SAVES a file via write_file, strong), {\"type\":\"gate\"} (pure routing; conditions on edges), {\"type\":\"fanout\",\"worker_prompt\":\"... {task} ...\",\"converge\":\"<nid>\"} (optional \"plan_prompt\" customizes the task planner; workers get web_search/web_fetch/read_file), {\"type\":\"code_review\",\"prompt\":\"...\",\"scope\":\"<opt glob>\"} (read-only code analysis: grep/glob/read, strong), {\"type\":\"code_edit\",\"prompt\":\"...\",\"files\":[\"<opt>\"]} (read+write+edit files, strong), {\"type\":\"shell_check\",\"command\":\"<cmd>\",\"timeout_secs\":<opt>} (run a read-only shell command like tests/build/lint, output captured as result), {\"type\":\"sub_agent\",\"task\":\"...\",\"tools\":[\"<opt>\"],\"model\":\"<opt>\"} (spawn a sub-agent for a self-contained task), {\"type\":\"notify\",\"message\":\"...\",\"channel\":\"<opt>\"} (send a notification to the user), {\"type\":\"wait\",\"seconds\":<opt>,\"until_condition\":\"<opt>\",\"timeout_secs\":<opt>} (pause execution or poll for a condition). There are no tools/handler/model fields — capability is fixed per kind. Execution walks a SINGLE path: each non-fanout node hands off to exactly ONE next node. Routing from a node with several outgoing edges: the executor takes a matching `condition` edge, and if none match (or several tie) it falls through to the lowest target id — it never stops. So put `condition`s on the branch edges AND include exactly one unconditional default edge as the catch-all. The ONLY parallelism is `fanout` — it runs workers then continues at `converge` (which also needs an edge into it). Do NOT author diamond fan-ins (e.g. a→c and b→c expecting both a and b to finish first) — only one path runs. Examples: (1) linear research→report — {\"id\":\"demo\",\"nodes\":[{\"id\":\"research\",\"kind\":{\"type\":\"research\",\"prompt\":\"Research the topic; list 5 key facts each with a source URL\"}},{\"id\":\"report\",\"kind\":{\"type\":\"report\",\"prompt\":\"Write a cited report from the findings and save it with write_file\"}}],\"edges\":[{\"source\":\"research\",\"target\":\"report\"}]}. (2) parallel fan-out — {\"id\":\"demo2\",\"nodes\":[{\"id\":\"plan\",\"kind\":{\"type\":\"research\",\"prompt\":\"Identify the sub-topics to cover\"}},{\"id\":\"work\",\"kind\":{\"type\":\"fanout\",\"worker_prompt\":\"Investigate {task}\",\"converge\":\"final\"}},{\"id\":\"final\",\"kind\":{\"type\":\"report\",\"prompt\":\"Synthesize the findings into a report and save it with write_file\"}}],\"edges\":[{\"source\":\"plan\",\"target\":\"work\"},{\"source\":\"work\",\"target\":\"final\"}]}. (3) code review→fix→verify — {\"id\":\"code\",\"nodes\":[{\"id\":\"review\",\"kind\":{\"type\":\"code_review\",\"prompt\":\"Find bugs in the auth module\"}},{\"id\":\"fix\",\"kind\":{\"type\":\"code_edit\",\"prompt\":\"Fix the issues found\"}},{\"id\":\"test\",\"kind\":{\"type\":\"shell_check\",\"command\":\"cargo test\"}},{\"id\":\"report\",\"kind\":{\"type\":\"report\",\"prompt\":\"Summarize changes and test results\"}}],\"edges\":[{\"source\":\"review\",\"target\":\"fix\"},{\"source\":\"fix\",\"target\":\"test\"},{\"source\":\"test\",\"target\":\"report\"}]}."
            });
