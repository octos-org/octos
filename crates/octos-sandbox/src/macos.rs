...
fn profile_literal(path: &Path) -> Result<String> {
    let path = path.to_str().ok_or_else(|| eyre::eyre!("sandbox path must be UTF-8"))?;
    eyre::ensure!(!path.chars().any(char::is_control), "sandbox path contains control characters");
    Ok(format!("\"{}\"", path.replace('\\', "\\\\").replace('"', "\\\"")))
}

pub(super) fn command(executable: &Path) -> Result<Command> {
    eyre::ensure!(Path::new("/usr/bin/sandbox-exec").is_file(), "macOS sandbox-exec is unavailable");
    ...
    let profile = format!("{WORKER_PROFILE}\n\
    ...
    unsafe { command.pre_exec(|| mark_descriptors_close_on_exec()); }
    Ok(command)
}
...
fn descriptors() -> io::Result<([libc::proc_fdinfo; 4096], usize)> {
    ...
    let bytes = unsafe { libc::proc_pidinfo(libc::getpid(), libc::PROC_PIDLISTFDS, 0, entries.as_mut_ptr().cast(), capacity) };
    if bytes <= 0 || bytes >= capacity { return Err(io::Error::from_raw_os_error(libc::EMFILE)); }
    Ok((entries, bytes as usize / std::mem::size_of::<libc::proc_fdinfo>()))
}
...
pub(super) fn confine() -> Result<HostSandbox> {
    ...
    let bytes = unsafe { libc::proc_pidinfo(libc::getpid(), libc::PROC_PIDTASKINFO, 0, (&mut info as *mut libc::proc_taskinfo).cast(), std::mem::size_of_val(&info) as i32) };
    eyre::ensure!(bytes as usize == std::mem::size_of_val(&info) && info.pti_threadnum == 1,
    ...
    for entry in &entries[..count] {
        if entry.proc_fd > 2 { unsafe { libc::close(entry.proc_fd); } }
    }
    ...
    if sandboxed > 0 {
        for operation in [c"network-outbound", c"network-inbound", c"process-fork"] {
            eyre::ensure!(unsafe { sandbox_check(libc::getpid(), operation.as_ptr(), 0) } > 0,
                "inherited sandbox allows {}", operation.to_string_lossy());
        }
        for operation in [c"file-read-data", c"file-read-metadata", c"file-write-data"] {
            eyre::ensure!(unsafe { sandbox_check(libc::getpid(), operation.as_ptr(), 1, c"/private/etc/passwd".as_ptr()) } > 0,
                "inherited sandbox permits unrelated files");
        }
        return Ok(HostSandbox { platform: "macos-seatbelt" });
    }
    eyre::ensure!(sandboxed == 0, "cannot inspect inherited Seatbelt confinement");
    let profile = CString::new(WORKER_PROFILE)?;
    let mut error = std::ptr::null_mut();
    let status = unsafe { sandbox_init(profile.as_ptr(), 0, &mut error) };
    if status != 0 {
        let message = if error.is_null() { "unknown Seatbelt error".to_owned() }
    ...
    Ok(HostSandbox { platform: "macos-seatbelt" })
}
