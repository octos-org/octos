...
use seccompiler::{BpfProgram, SeccompAction, SeccompCmpArgLen, SeccompCmpOp,
...
fn ruleset() -> Result<OwnedFd> {
    let mut rules = Ruleset::default()
        ...
        .create()?;
    rules = rules
        .add_rules(path_beneath_rules(["/dev/null"], AccessFs::ReadFile | AccessFs::WriteFile))?
        .add_rules(path_beneath_rules(["/dev/urandom", "/dev/random"], AccessFs::ReadFile))?;
...
fn filter(allow_entry: bool) -> Result<BpfProgram> {
    ...
    let mut calls = vec![
        ...
        libc::SYS_mmap, libc::SYS_mprotect, libc::SYS_munmap, libc::SYS_mremap,
        libc::SYS_madvise, libc::SYS_brk, libc::SYS_futex, libc::SYS_futex_waitv,
        libc::SYS_set_tid_address, libc::SYS_set_robust_list, libc::SYS_rseq,
        libc::SYS_rt_sigaction, libc::SYS_rt_sigprocmask, libc::SYS_rt_sigreturn,
        libc::SYS_sigaltstack, libc::SYS_signalfd4, libc::SYS_pselect6, libc::SYS_ppoll,
        libc::SYS_epoll_create1, libc::SYS_epoll_ctl, libc::SYS_epoll_pwait,
        libc::SYS_epoll_pwait2, libc::SYS_eventfd2, libc::SYS_timerfd_create,
        libc::SYS_timerfd_settime, libc::SYS_timerfd_gettime,
        libc::SYS_clock_gettime, libc::SYS_clock_getres, libc::SYS_clock_nanosleep,
        libc::SYS_nanosleep, libc::SYS_gettimeofday, libc::SYS_getrandom,
        libc::SYS_getpid, libc::SYS_gettid, libc::SYS_getppid,
        libc::SYS_getuid, libc::SYS_geteuid, libc::SYS_getgid, libc::SYS_getegid,
        libc::SYS_getresuid, libc::SYS_getresgid, libc::SYS_getgroups,
        libc::SYS_uname, libc::SYS_sysinfo, libc::SYS_getrusage,
        libc::SYS_getrlimit, libc::SYS_prlimit64, libc::SYS_sched_getaffinity,
        libc::SYS_sched_yield, libc::SYS_sched_getparam, libc::SYS_sched_getscheduler,
        libc::SYS_exit, libc::SYS_exit_group,
        // The worker must be able to install its stricter second profile.
        libc::SYS_landlock_create_ruleset, libc::SYS_landlock_add_rule,
        libc::SYS_landlock_restrict_self, libc::SYS_seccomp,
    ];
    ...
    if allow_entry {
        ...
        let write_flags = libc::O_ACCMODE | libc::O_CREAT | libc::O_TRUNC | libc::O_APPEND
            | (libc::O_TMPFILE & !libc::O_DIRECTORY);
        rules.insert(libc::SYS_openat, vec![SeccompRule::new(vec![
            SeccompCondition::new(2, SeccompCmpArgLen::Dword, SeccompCmpOp::MaskedEq(write_flags as u64), 0)?,
        ])?]);
    }
    ...
    let permitted = required | (libc::CLONE_FS | libc::CLONE_FILES | libc::CLONE_SYSVSEM
        | libc::CLONE_SETTLS | libc::CLONE_PARENT_SETTID | libc::CLONE_CHILD_CLEARTID
        | libc::CLONE_CHILD_SETTID | libc::CLONE_DETACHED) as u64;
    rules.insert(libc::SYS_clone, vec![SeccompRule::new(vec![
        SeccompCondition::new(0, SeccompCmpArgLen::Qword, SeccompCmpOp::MaskedEq(required), required)?,
        SeccompCondition::new(0, SeccompCmpArgLen::Qword, SeccompCmpOp::MaskedEq(!permitted), 0)?,
    ])?]);
    let prctl_ops = [libc::PR_SET_NAME, libc::PR_GET_NAME, libc::PR_GET_NO_NEW_PRIVS];
    let mut prctl = prctl_ops.into_iter().map(|op| SeccompRule::new(vec![
    ...
    Ok(SeccompFilter::new(rules, SeccompAction::Errno(libc::ENOSYS as u32),
        SeccompAction::Allow, std::env::consts::ARCH.try_into()?)?.try_into()?)
}
...
fn install(rules: &OwnedFd, filter: &BpfProgram) -> io::Result<()> {
    ...
    let program = libc::sock_fprog {
        len: filter.len().try_into().map_err(|_| io::Error::from_raw_os_error(libc::EINVAL))?,
        filter: filter.as_ptr().cast::<libc::sock_filter>().cast_mut(),
    };
    if unsafe { libc::syscall(libc::SYS_seccomp, libc::SECCOMP_SET_MODE_FILTER, 0, &program) } != 0 {
        return Err(io::Error::last_os_error());
    }
    Ok(())
}
...
pub(super) fn command(executable: &Path) -> Result<Command> {
    let helper = ["/usr/bin/bwrap", "/bin/bwrap"].into_iter()
        .find(|path| Path::new(path).is_file())
        .ok_or_else(|| eyre::eyre!("host-managed Linux agents require bubblewrap (bwrap) and enabled user namespaces"))?;
    let mounts = runtime_files(executable)?;
    let filter = filter(true)?;
    let raw_fd = unsafe { libc::memfd_create(c"octos-host-seccomp".as_ptr(), libc::MFD_CLOEXEC | libc::MFD_ALLOW_SEALING) };
    eyre::ensure!(raw_fd >= 0, "create host-managed seccomp descriptor: {}", io::Error::last_os_error());
    ...
    eyre::ensure!(unsafe { libc::fcntl(raw_fd, libc::F_ADD_SEALS, seals) } == 0, "seal seccomp descriptor");
    let mut command = Command::new(helper);
    command.args(["--unshare-all", "--unshare-user", "--disable-userns", "--die-with-parent",
        "--new-session", "--clearenv", "--cap-drop", "ALL", "--hostname", "octos-worker", "--chdir", "/"]);
    let mut search_paths = BTreeSet::new();
    for (destination, source) in mounts {
        command.arg("--ro-bind").arg(source).arg(&destination);
        if let Some(parent) = destination.parent() { search_paths.insert(parent.to_path_buf()); }
    }
    ...
    if !search_paths.is_empty() {
        command.args(["--setenv", "LD_LIBRARY_PATH"]).arg(std::env::join_paths(search_paths)?);
    }
    for device in ["/dev/null", "/dev/random", "/dev/urandom"] {
        command.args(["--ro-bind", device, device]);
    }
    command.arg("--ro-bind").arg(executable).arg("/app/octos")
        .args(["--remount-ro", "/", "--seccomp", &raw_fd.to_string(), "--", "/app/octos"]);
...
fn runtime_files(executable: &Path) -> Result<BTreeMap<PathBuf, PathBuf>> {
    let tree = lddtree::DependencyAnalyzer::new(PathBuf::from("/")).library_paths(vec![])
        .analyze(executable).wrap_err("analyze host-managed ELF dependencies")?;
    let mut mounts = BTreeMap::new();
    let mut paths: Vec<PathBuf> = tree.libraries.values().map(|lib| lib.path.clone()).collect();
    if let Some(interpreter) = tree.interpreter { paths.push(PathBuf::from(interpreter)); }
    for path in paths {
        eyre::ensure!(path.is_absolute() && path.components().all(|part|
        ...
        let real = path.canonicalize().wrap_err("resolve ELF runtime dependency")?;
        eyre::ensure!(["/lib", "/lib64", "/usr/lib", "/usr/lib64"].iter().any(|root| real.starts_with(root)),
            "host-managed executables must use system runtime libraries");
        let metadata = real.metadata()?;
        eyre::ensure!(metadata.is_file() && metadata.uid() == 0 && metadata.mode() & 0o022 == 0,
        ...
        for ancestor in real.ancestors().skip(1) {
            ...
            eyre::ensure!(metadata.uid() == 0 && metadata.mode() & 0o022 == 0,
...
pub(super) fn confine() -> Result<HostSandbox> {
    ...
    eyre::ensure!(unsafe { libc::syscall(libc::SYS_gettid) } == unsafe { libc::getpid() } as libc::c_long,
    ...
    Ok(HostSandbox { platform: "linux-bubblewrap-landlock-seccomp" })
}
