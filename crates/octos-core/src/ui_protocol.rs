...
pub mod progress_kinds {
    ...
    /// `progress/updated{kind:"status_word", label:"<word>"}` once
    /// every ~8s during an in-flight agent turn; the word is sourced
    /// from the per-persona pool generated by `PersonaService`
    /// (CJK-aware: Chinese words for Chinese input, English for
    /// English). The SPA bridge (`ui-protocol-event-router.ts`) lifts
    /// these onto a `crew:status_word` DOM event the
    /// `ThinkingIndicator` component listens for.
...
mod tests {
    ...
    #[test]
    fn progress_updated_status_word_wire_shape() {
        // Lock the `progress/updated{kind:"status_word"}` wire shape
        // the SPA `ui-protocol-event-router.ts` already parses (via
        // the `progress/updated` bridge guard) and dispatches as
        // `crew:status_word`. The rotating creative word travels in
        // `metadata.label`; no other field is set for the steady-state
        // rotation case so the envelope stays compact.
        let mut metadata = UiProgressMetadata::new(progress_kinds::STATUS_WORD);
        metadata.label = Some("Pondering".into());
        ...
        // Round-trip back through the typed enum so future wire-shape
        // edits surface in the assertion above.
        let decoded_notification: RpcNotification<Value> =
