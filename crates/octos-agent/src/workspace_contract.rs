...
pub async fn enforce_spawn_task_contract(
    ...
    let artifact_paths = match resolve_artifacts(
        ...
        Ok(paths) => paths,
        Err(error) => {
            run_failure_actions(workspace_root, &task_policy.on_failure);
...
fn run_failure_actions(workspace_root: &Path, actions: &[String]) {
    for action in actions {
        if let Some(pattern) = action.strip_prefix("cleanup:") {
...
mod tests {
    ...
    async fn tts_contract_auto_sends_new_mp3() {
        ...
        let result =
            enforce_spawn_task_contract(&tools, "fm_tts", "tool-call-1", &[], UNIX_EPOCH).await;
    ...
    async fn tts_contract_fails_when_no_mp3_exists() {
        ...
        let result =
            enforce_spawn_task_contract(&tools, "fm_tts", "tool-call-2", &[], UNIX_EPOCH).await;
