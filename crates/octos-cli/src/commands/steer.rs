...
mod tests {
    ...
    /// against the SAME instance data dir and assert a steer continuation
    /// is enqueued — the cross-process contract end to end (CLI process ≠
    /// serve process), not a same-process fixture.
    #[test]
    fn olp_ctrl_steer_subprocess_cli_to_sweep() {
        let temp = tempfile::tempdir().expect("tempdir");
        let cwd = temp.path().join("proj");
        std::fs::create_dir_all(&cwd).expect("cwd");
        ...
        std::fs::create_dir_all(transcript.parent().expect("parent")).expect("mkdir");
        std::fs::write(&transcript, "{}\n").expect("seed transcript");
        // Instance data dir (CLI's resolve_profile_data_root fallback
        // layout: <state_home>/profiles/octos/data) under OCTOS_HOME.
        ...
        // Locate the just-built octos binary (integration env var when
        // available, else the debug target path).
        let cli = option_env!("CARGO_BIN_EXE_octos")
        ...
        // Files landed in the instance inbox.
        let safe = crate::autonomy::hash_session_for_inbox(session);
        let notes = instance_data
            .join("inbox")
            .join(format!("{safe}.reviewer-notes"));
        assert!(notes.exists(), "reviewer-notes written by the subprocess");
        // Serve-side sweep on the SAME instance data dir enqueues a
        // continuation.
        ...
        orchestrator.steer_inbox_sweep(&instance_data, "octos");
        let after = orchestrator.pending_steer_continuation_count_for_test(&key, "octos");
        assert_eq!(
            after,
            before + 1,
            "sweep enqueues the subprocess-queued steer"
        );
    }
