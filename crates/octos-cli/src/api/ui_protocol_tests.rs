...
fn appui_loop_uses_response_content_not_session_history_for_self_paced() {
    ...
    // background row.
    let fallback_only = appui_history_last_non_empty_assistant_after(&history, pre_assistant_count);
    assert_eq!(
    ...
    let session_id = SessionKey::with_profile("tenant-a", "api", "appui-loop-self-paced-response");
...
async fn advertised_stdio_methods_have_dispatch_path() {
    assert_advertised_stdio_methods_have_dispatch_path(Arc::new(AppState::empty_for_tests())).await;

    let dir = tempfile::tempdir().unwrap();
    assert_advertised_stdio_methods_have_dispatch_path(Arc::new(local_profile_state(dir.path())))
        .await;
}
...
async fn memory_and_cron_rpc_methods_forward_rest_panel_bodies() {
    let dir = tempfile::tempdir().unwrap();
    let profile_store = Arc::new(crate::profiles::ProfileStore::open_unified(dir.path()).unwrap());
...
fn task_list_entry_from_value_round_trips_m13b_projection_fields() {
    ...
    let entry = task_list_entry_from_value(raw).expect("M13-B projection JSON must deserialize");
...
fn task_list_entry_from_value_tolerates_legacy_supervisor_json() {
    ...
    let entry = task_list_entry_from_value(raw).expect("legacy supervisor JSON must deserialize");
...
fn production_autonomy_rpc_evidence_writes_non_fixture_ledgers() {
    ...
    let goal_result = raw_autonomy_rpc(&goal_set, features, Some(test_profile)).expect("goal set");
    let goal_notifs = autonomy_rpc_evidence_to_dir(path, methods::SESSION_GOAL_SET, &goal_result);
    assert!(
        matches!(
    ...
    let fire_result = raw_autonomy_rpc(&fire_now, features, Some(test_profile)).expect("loop fire");
    ...
    assert!(
        agent_ledger.contains("\"agent_started\"") && agent_ledger.contains("\"agent_completed\""),
...
fn m15_raw_autonomy_rpc_dispatches_every_method_to_orchestrator() {
    ...
    for (method, params, expected_call) in &cases {
        ...
        let result = raw_autonomy_rpc_with_orchestrator(&request, features, None, &orchestrator)
...
fn task_artifact_alias_accepts_task_id_and_agent_id() {
    ...
    let result = raw_autonomy_rpc_with_orchestrator(&list_with_both, features, None, &orchestrator)
...
fn task_artifact_alias_task_id_requires_session_id() {
    ...
    let err = raw_autonomy_rpc_with_orchestrator(&list_no_session, features, None, &orchestrator)
    ...
    let err = raw_autonomy_rpc_with_orchestrator(&read_no_session, features, None, &orchestrator)
...
fn session_opened_notification_capabilities_are_filtered_for_ingress() {
    ...
    let UiProtocolLedgerEvent::Notification(UiNotification::SessionOpened(opened)) = event else {
...
#[tokio::test]
async fn live_forwarder_routes_background_child_as_canonical_v2() {
    let (ws, mut rx) = ws_connection_for_test(16);
    let ledger = Arc::new(UiProtocolLedger::new(16));
    ...
    // The post-commit observer emits one child completion envelope.
    ledger.append_notification(background_child_v2_for(&session_id));

    let frame = tokio::time::timeout(std::time::Duration::from_secs(1), rx.recv())
        .await
        .expect("expected the background child envelope within 1s")
        .expect("ws still open");
    assert_eq!(
        frame_method(&frame).as_deref(),
        Some("projection/envelope"),
        "background completion is always a projection envelope",
    );
    let frame: Value = match frame {
        WsMessage::Text(text) => serde_json::from_str(&text).expect("valid child JSON"),
        other => panic!("expected text frame, got {other:?}"),
    };
    assert_eq!(
        frame["params"]["payload"]["type"],
    ...
    assert!(
        rx.try_recv().is_err(),
        "one committed child result produces one wire envelope",
    );

    abort_live_forwarders(&forwarders, &ledger).await;
}

/// Background persistence carries explicit projection context only within
/// its task-local scope; ordinary commits remain foreground projections.
#[tokio::test]
async fn background_projection_override_is_task_local() {
    assert!(MESSAGE_PROJECTION_OVERRIDE.try_with(Clone::clone).is_err());
    let context = BackgroundChildProjection {
        parent_turn_id: "parent-turn".into(),
        response_to_client_message_id: Some("cmid-parent".into()),
        task_id: Some("task-child".into()),
        tool_call_id: Some("call-child".into()),
        media: vec!["artifact.md".into()],
    };
    let observed = MESSAGE_PROJECTION_OVERRIDE
        .scope(
            Some(MessageProjectionOverride::BackgroundChild(context.clone())),
            async { MESSAGE_PROJECTION_OVERRIDE.with(Clone::clone) },
        )
        .await;
    assert!(matches!(
        observed,
        Some(MessageProjectionOverride::BackgroundChild(actual))
            if actual.parent_turn_id == context.parent_turn_id
                && actual.task_id == context.task_id
                && actual.media == context.media
    ));
    assert!(MESSAGE_PROJECTION_OVERRIDE.try_with(Clone::clone).is_err());
}
        ...
        .scope(
            Some(MessageProjectionOverride::BackgroundChild(
                BackgroundChildProjection {
                    parent_turn_id: thread_id.to_owned(),
                    response_to_client_message_id: None,
                    task_id: Some("task-fm-tts".into()),
                    tool_call_id: None,
                    media: media.clone(),
                },
            )),
...
/// ack on the old live persisted-message broadcast, but
...
/// persist + canonical child envelope), so the legitimate
...
///   2. Exactly ONE `assistant_persisted` v2 envelope fires for the
...
async fn synth_ack_not_persisted_to_jsonl_when_spawn_only() {
    ...
    // Invariant 2: exactly ONE canonical assistant-persisted envelope
    // fires. The skipped ack has no post-commit projection at all.
    let mut assistant_envelopes = Vec::new();
    let mut background_children = 0usize;
    while let Ok(event) = subscriber.try_recv() {
        if let UiProtocolLedgerEvent::Notification(UiNotification::EnvelopeV2(envelope)) =
            &event.event
        {
            match &envelope.envelope.payload {
                PayloadV2::AssistantPersisted { text, .. } => {
                    assistant_envelopes.push(text.clone());
                }
                PayloadV2::BackgroundChildCompleted { .. } => {
    ...
    assert_eq!(
        ...
        1,
        "exactly ONE assistant-persisted envelope must fire (the preamble); \
             pre-fix TWO persisted rows existed; got {}",
        assistant_envelopes.len(),
    );
    assert_eq!(
        assistant_envelopes[0], preamble_text,
        "the surviving envelope must be the preamble",
    );
    assert_eq!(
        background_children, 0,
        "the skipped ack cannot fabricate a background child",
    );
...
///   2. Exactly ONE v2 `assistant_persisted` envelope fires for the
///      preamble — no child envelope appears because the persist that would have
///      triggered the observer never happens.
#[tokio::test(flavor = "current_thread")]
async fn synth_ack_skip_invariants_hold_for_each_spawn_only_tool_name() {
    ...
    for tool_name in tool_names {
        ...
        let session_id = SessionKey(format!("local:spawn-only-synth-ack-no-persist-{tool_name}"));
        let mut subscriber = ledger.subscribe(&session_id);
        ...
        // Invariant B: exactly one assistant-persisted v2 envelope fires
        // regardless of `<tool_name>`. The ack persist is skipped, so the
        // `MessageCommitObserver` never receives it.
        let mut assistant_envelopes = Vec::new();
        let mut background_children = 0usize;
        while let Ok(event) = subscriber.try_recv() {
            if let UiProtocolLedgerEvent::Notification(UiNotification::EnvelopeV2(envelope)) =
        ...
        assert_eq!(
            assistant_envelopes.len(),
            1,
            "exactly ONE assistant-persisted envelope must fire for spawn_only \
                 tool `{tool_name}`; got {}",
            assistant_envelopes.len(),
        );
        assert_eq!(
        ...
        assert_eq!(
            background_children, 0,
            "no child can be emitted for `{tool_name}` because the ack is skipped",
        );
...
/// Codex round-3 P2 acceptance: when a spawn_only self-paced
/// turn has a preamble carrying a `<<loop-next-in: 60s>>` hint,
/// the captured_final_reply must end up as the PREAMBLE text
/// (not the synthesised ack) so `apply_self_paced_response`
/// parses the model-requested 60s delay — NOT the 15-min
/// default — and stamps `next_run_at_ms` accordingly.
///
/// This pins the integration shape: feed the preamble text
/// (which `captured_final_reply_for_synth_ack_skip` returns
/// when a preamble is persisted) through
/// `apply_self_paced_response` and assert the parsed delay is
...
fn self_paced_loop_honours_preamble_hint_when_spawn_only_skipped() {
    ...
    let orchestrator = InProcessAgentOrchestrator::default();
    let session_id = SessionKey::with_profile("tenant-round3", "api", "self-paced-preamble-hint");
    let created = orchestrator
        .create_loop(LoopCreateRequest {
            session_id: session_id.clone(),
            profile_id: "tenant-round3".into(),
            prompt: Some("watch the build".into()),
            command: None,
            interval_seconds: None,
            mode: Some("self_paced".into()),
        })
    ...
    // Wire shape: a preamble with hint AND a synth ack present.
    // The helper picks the preamble.
    let preamble = "Starting the pipeline. <<loop-next-in: 60s>>";
    let synth_ack = "Background work started for `run_pipeline`. \
                         The final result will be delivered automatically when it is ready."
        .to_string();
    let captured = captured_final_reply_for_synth_ack_skip(synth_ack, Some(preamble));
    assert_eq!(
        captured, preamble,
        "helper must pick the preamble text when present",
    );
    ...
    let parsed = orchestrator
        .apply_self_paced_response(&loop_id, "tenant-round3", &captured)
        .expect("apply_self_paced_response on preamble");
    assert_eq!(
        parsed,
        Some(std::time::Duration::from_secs(60)),
        "preamble carried `<<loop-next-in: 60s>>` — parser MUST return 60s, NOT the 15-min default",
    );

    // Verify next_run_at_ms is ~60s ahead (NOT 900s) — the
    // model-requested delay deterministically beats the
    // history-fallback race.
    ...
    let record = loops
        ...
        .expect("loop record present");
    let next = record
        .get("next_run_at_ms")
        .and_then(|v| v.as_i64())
        .expect("self-paced loop must have a fresh next_run_at_ms");
    let delta_ms = next - before;
    assert!(
        (50_000..=70_000).contains(&delta_ms),
        "next_run_at_ms must be ~60s in the future (got {delta_ms} ms) — proves the preamble's \
             `<<loop-next-in: 60s>>` hint beat the default 15-min fallback",
    );
}
...
fn self_paced_loop_reschedules_when_only_assistant_content_is_synth_ack() {
    ...
    // `list_loops`.
    let orchestrator = InProcessAgentOrchestrator::default();
    let session_id = SessionKey::with_profile("tenant-new03", "api", "self-paced-bare-spawn-only");
    let created = orchestrator
        .create_loop(LoopCreateRequest {
            session_id: session_id.clone(),
...
/// V2 projection sequence is per thread while cursor sequencing is per
/// durable session stream. A pre-existing ledger row proves the two are
/// stamped independently.
#[tokio::test]
async fn ledger_stamps_v2_projection_seq_and_cursor_independently() {
    let ledger = UiProtocolLedger::new(8);
    let session_id = SessionKey("local:seq".into());

    let seed = ledger.append_notification(assistant_persisted_v2_for(&session_id));
    let first = ledger
        .emit_envelope_v2(
            &session_id,
            "thread-seq".into(),
            PayloadV2::AssistantDelta {
                text: "one".into(),
                assistant_segment_id: "thread-seq:assistant:1".into(),
            },
            None,
        )
        .expect("first v2 envelope");
    let second = ledger
        .emit_envelope_v2(
            &session_id,
            "thread-seq".into(),
        ...
        .expect("second v2 envelope");

    let first_envelope = match &first.event {
        UiProtocolLedgerEvent::Notification(UiNotification::EnvelopeV2(event)) => &event.envelope,
        other => panic!("expected v2 envelope, got {other:?}"),
    };
    let second_envelope = match &second.event {
        UiProtocolLedgerEvent::Notification(UiNotification::EnvelopeV2(event)) => &event.envelope,
        other => panic!("expected v2 envelope, got {other:?}"),
    };
    assert_eq!(first_envelope.seq, 1);
    assert_eq!(second_envelope.seq, 2);
    assert_eq!(first_envelope.cursor.as_ref(), Some(&first.cursor));
    assert_eq!(second_envelope.cursor.as_ref(), Some(&second.cursor));
    assert!(seed.cursor.seq < first.cursor.seq);
    assert!(first.cursor.seq < second.cursor.seq);
}
...
#[tokio::test]
async fn live_forwarder_delivers_background_child_without_legacy_fallback() {
    let (ws, mut rx) = ws_connection_for_test(16);
    let ledger = Arc::new(UiProtocolLedger::new(16));
    ...
    let frame = tokio::time::timeout(std::time::Duration::from_secs(1), rx.recv())
        .await
        .expect("expected the canonical background child within 1s")
        .expect("ws still open");
    assert_eq!(
        frame_method(&frame).as_deref(),
        Some("projection/envelope"),
        "first frame must be the canonical projection envelope",
    );

    tokio::time::sleep(std::time::Duration::from_millis(50)).await;
    assert!(
        rx.try_recv().is_err(),
        "background completion has no legacy companion frame",
    );
...
async fn live_forwarder_emits_event_appended_between_replay_and_forwarder_install() {
    ...
    let initial = ledger.append_notification(assistant_persisted_v2_for(&session_id));
    ...
    let gap = ledger.append_notification(assistant_persisted_v2_for(&session_id));
...
async fn send_notification_durable_does_not_double_deliver_via_live_forwarder() {
    ...
    send_notification_durable(&ws, &ledger, assistant_persisted_v2_for(&session_id))
    ...
    let first = tokio::time::timeout(std::time::Duration::from_secs(1), rx.recv())
        ...
        .expect("first frame")
        .expect("ws open");
    assert_eq!(frame_method(&first).as_deref(), Some("projection/envelope"));
    ...
    let forwarders_other: SharedLiveForwarders = Arc::new(tokio::sync::Mutex::new(HashMap::new()));
...
async fn state_with_profile_llm_and_sandbox(
    ...
    let profile_runtime =
        make_m11e_profile_with_llm_and_sandbox(profile_id, &profile_data_dir, llm, sandbox).await;
...
async fn second_session_open_with_new_cwd_reports_cached_workspace_root() {
    let temp = tempfile::tempdir().expect("tempdir");
    let (state, _profile_runtime) = state_with_profile(temp.path(), "m11e-rebind-attempt").await;
...
async fn appui_session_without_client_cwd_respects_operator_default_session_cwd() {
    ...
    let (mut state_inner, profile_runtime) = {
        let (state, profile_runtime) = state_with_profile(temp.path(), "m11f-tier2-default").await;
    ...
    let session_id = SessionKey::with_profile("m11f-tier2-default", "api", "tier2-no-client-cwd");
    ...
    let session_runtime = state
        ...
        .expect("cached session runtime");
    assert_eq!(
        std::fs::canonicalize(&session_runtime.workspace_root).expect("canonicalize runtime root"),
        std::fs::canonicalize(&operator_default).expect("canonicalize operator default"),
...
fn structurally_huge_string_array_is_truncated_to_valid_under_cap_json() {
    ...
    let parsed: Value = serde_json::from_str(&out).expect("array-truncated frame is valid JSON");
...
fn minimal_rpc_error_frame_is_tiny_valid_and_same_id() {
    ...
    let frame = minimal_rpc_error_frame(Some(id.clone()), "response payload too large to deliver")
...
fn minimal_rpc_error_frame_with_huge_id_returns_none_not_truncated_id() {
    ...
    assert!(
        minimal_rpc_error_frame(Some(huge_id), "response payload too large to deliver").is_none(),
...
async fn send_minimal_rpc_error_fallback_huge_id_closes_1011_and_fails_no_loop() {
    ...
    assert!(
        minimal_rpc_error_frame(Some(huge_id.clone()), RPC_RESPONSE_TOO_LARGE_MESSAGE).is_none(),
...
/// RFC-1 fixup (codex P2): the per-turn `tool_registry.snapshot_excluding(&[])`
/// path in `process_chat_message_streaming` clones the
/// `Arc<dyn Tool>` instances. Without an explicit re-registration,
/// the per-turn dispatcher object is the SAME shared instance as
/// the cached session registry's dispatcher. Any subsequent
/// `wire_mofa_make_registry_back_ref` call would mutate the
/// shared object's `Mutex<Weak<ToolRegistry>>` and other
/// overlapping turns/background `mofa_make` runs would resolve
/// through the WRONG registry.
///
/// This test simulates the snapshot pattern the production path
/// uses and asserts that after the per-turn re-registration
/// (which mirrors how `activate_tools` is re-registered just
/// above in the same file) the per-turn dispatcher is a SEPARATE
/// object from the cached one — so wiring one's Weak does NOT
/// touch the other's.
#[test]
fn per_turn_snapshot_creates_fresh_dispatcher() {
    ...
    // does during `SessionRuntime::bootstrap`.
    let mut parent = ToolRegistry::new();
    let entry = MakeTypeEntry::new("slides", "mofa-slides", "mofa_slides", "Slides description");

    let parent_dispatcher = MofaMakeTool::new();
    parent_dispatcher.register_or_replace(entry.clone());
    ...
    // Wire the parent dispatcher's Weak to a parent-Arc registry —
    // this represents the SessionRuntime's cached registry wiring.
    let parent_arc = Arc::new(parent);
    let parent_dispatcher_back = parent_arc
        .get("mofa_make")
        .and_then(|t| t.as_any().downcast_ref::<MofaMakeTool>())
        .expect("parent mofa_make registered");
    parent_dispatcher_back.set_registry(Arc::downgrade(&parent_arc));

    // Capture identity (Arc data pointer) of the parent dispatcher.
    let parent_disp_ptr = Arc::as_ptr(&parent_arc.get("mofa_make").cloned().unwrap()) as *const ();

    // Simulate the per-turn snapshot path. The cloned Arc points
    // to the SAME dispatcher object as the parent's (this is the
    // bug codex flagged).
    let mut per_turn = parent_arc.snapshot_excluding(&[]);
    let cloned_disp_ptr = Arc::as_ptr(&per_turn.get("mofa_make").cloned().unwrap()) as *const ();
    assert_eq!(
        cloned_disp_ptr, parent_disp_ptr,
        "snapshot_excluding clones Arc references — pre-fix sanity \
             check that the per-turn arc IS the same object as the \
             parent's (this is the share-mutate hazard)"
    );

    // Apply the production fixup: mint fresh dispatcher + describe
    // instances seeded from the existing dispatcher's catalog, and
    // re-register on the per-turn registry. This is what the
    // production path in ui_protocol.rs does post-snapshot.
    if let Some(existing) = per_turn.get("mofa_make").cloned() {
        let dispatcher = existing
            .as_any()
            .downcast_ref::<MofaMakeTool>()
            .expect("dispatcher downcast");
        let entries = dispatcher.entries();
        let fresh_dispatcher = MofaMakeTool::new();
        for entry in &entries {
            fresh_dispatcher.register_or_replace(entry.clone());
        }
        per_turn.register(fresh_dispatcher);

        if per_turn.get("mofa_describe_content_type").is_some() {
            let fresh_describe = MofaDescribeContentTypeTool::new();
            for entry in &entries {
                fresh_describe.register_or_replace(entry.clone());
            }
            per_turn.register(fresh_describe);
        }
    }

    // Post-fixup identity assertion: the per-turn dispatcher is a
    // DIFFERENT Arc object than the parent's. So wiring the
    // per-turn dispatcher's Weak no longer touches the parent's.
    let new_disp_ptr = Arc::as_ptr(&per_turn.get("mofa_make").cloned().unwrap()) as *const ();
    assert_ne!(
        new_disp_ptr, parent_disp_ptr,
        "post-fixup per-turn dispatcher must be a SEPARATE Arc \
             object from the parent's — share-mutate hazard closed"
    );

    // And the catalog of the fresh dispatcher must mirror the
    // parent's — the LLM still sees the same content_type enum.
    let per_turn_dispatcher = per_turn
        .get("mofa_make")
        .and_then(|t| t.as_any().downcast_ref::<MofaMakeTool>())
        .expect("per-turn mofa_make registered");
    let entries = per_turn_dispatcher.entries();
    assert_eq!(entries.len(), 1, "catalog must be propagated");
    assert_eq!(entries[0].content_type, "slides");
    assert_eq!(entries[0].target_tool, "mofa_slides");

    // Critical: wire the per-turn dispatcher's Weak to the per-turn
    // registry and confirm the parent's dispatcher's Weak still
    // points at the parent registry (i.e. has NOT been mutated).
    let per_turn_arc = Arc::new(per_turn);
    let per_turn_dispatcher_back = per_turn_arc
        .get("mofa_make")
        .and_then(|t| t.as_any().downcast_ref::<MofaMakeTool>())
        .expect("per-turn mofa_make registered");
    per_turn_dispatcher_back.set_registry(Arc::downgrade(&per_turn_arc));

    // The parent dispatcher's Weak must still upgrade to the
    // parent registry. If the share-mutate hazard were live, this
    // would now point at the per-turn registry or be dropped.
    let parent_after = parent_arc
        .get("mofa_make")
        .and_then(|t| t.as_any().downcast_ref::<MofaMakeTool>())
        .expect("parent mofa_make still registered");
    // Use a dispatch call on the parent registry to verify
    // end-to-end. The parent registry doesn't have `mofa_slides`
    // registered (we didn't add it above) so the call surfaces a
    // DISPATCHER_ERROR specifically mentioning the missing target —
    // which proves the parent's Weak still resolves to the parent
    // registry. If the per-turn registry's Weak had clobbered the
    // parent's, the call would either (a) succeed by reaching some
    // per-turn-only tool, (b) error with "tool registry is not
    // available" (Weak dropped). Neither matches the expected
    // parent-registry behaviour.
    let rt = tokio::runtime::Runtime::new().unwrap();
    let result = rt.block_on(parent_after.execute(&serde_json::json!({
        "content_type": "slides",
        "args": {}
    })));
    let result = result.unwrap();
    // The parent's registry has the dispatcher catalog but no
    // mofa_slides target tool. The error message must mention
    // mofa_slides (proving we routed through the parent's catalog +
    // parent's get() lookup, not the per-turn's).
    assert!(
        result.output.contains("[DISPATCHER_ERROR]") && result.output.contains("mofa_slides"),
        "parent dispatcher must still resolve via the parent registry's \
