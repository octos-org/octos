...
pub fn build_router(state: Arc<AppState>) -> Router {
    ...
    let my_api = Router::new()
        ...
        // ui-protocol consolidation (octos #1621 follow-up): `/api/my/memory`,
        // `/api/my/memory/entities/{name}`, `/api/my/cron`, and
        // `/api/my/cron/{job_id}/enabled` retired in favor of WS RPC methods
        // `memory/overview`, `memory/entity`, `cron/list`, and `cron/toggle`
        // on `/api/ui-protocol/ws` (same shape as the M12 Phase D-5 content/*
        // retirement below). The panel handler cores in `memory_panel.rs` /
        // `cron_panel.rs` stay — the RPC dispatchers call them directly.
