...
test.describe("M9 protocol — fault injection", () => {
  ...
  test("drop mid-stream and reconnect with last cursor: replay restores state without duplicates", async () => {
    ...
    // server-side so the ledger contains turn/started -> ... -> the
    // turn_terminal envelope, then drop. We capture `lastCursorBeforeDrop =
    // baseline_open_cursor` so the resume HAS to replay the rest of the
    // ledger.
    ...
    try {
      ...
      await waitForTurnTerminal(c1, turnId, 45_000);
    } finally {
      await c1.close();
    }
    ...
    try {
      ...
      // same turn_id. We assert at least one of {turn/started, the
      // turn_terminal envelope} surfaces — the exact set depends on which
      // events the runtime ledgers (this is wire-level coverage of the
      // replay path itself).
  ...
  test("double turn/interrupt drains to one interrupted terminal event", async () => {
    ...
    try {
      ...
      const terminals = client.notificationsLog().filter((n) => isTurnTerminal(n, turnId));
