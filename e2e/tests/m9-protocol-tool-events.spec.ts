 ...
 * by `tool_call_id` and the same `turn_id`/`session_id` (the raw
 * `tool/started` / `tool/completed` frames are never delivered since #2318).
 *
...
test.describe("M9 protocol — tool events", () => {
  ...
  test("tool_start -> tool_end envelope correlation by tool_call_id", async () => {
    ...
    try {
      ...
      const terminal = await waitForTurnTerminal(client, turnId, 50_000);
      expect(terminal.params.turn_id).toBe(turnId);

      const log = client.notificationsLog();
      const toolStarted = envelopePayloads(log, "tool_start", turnId);
      const toolCompleted = envelopePayloads(log, "tool_end", turnId);
      ...
      // Each tool_start must be matched by a tool_end with the same
      // tool_call_id. Tools may interleave with progress events, so
      ...
      for (const id of startedIds) {
        expect(completedIds.has(id), `missing tool_end for ${id}`).toBe(true);
      }

      // For each correlation id, started must precede completed in the
      // notification log (durable ordering).
      for (const id of startedIds) {
        const startIdx = log.findIndex((n) => isEnvelopePayload(n, "tool_start", turnId, id));
        const endIdx = log.findIndex((n) => isEnvelopePayload(n, "tool_end", turnId, id));
